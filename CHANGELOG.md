...
- Nuxt 2 dev-server detection (`checkNuxtDevServer.ts`) no longer misclassifies a production build as `nuxt-dev`: the unhashed-entry marker now requires the full `/_nuxt/app.js` suffix (was matching any `app.js` basename, e.g. `/assets/app.js`), and the webpack-HMR marker requires a complete `__webpack_hmr` path segment instead of a bare substring match.
...
- New `react-dev` tech: detects a React dev server (Vite `npm run dev`, or CRA/custom `webpack-dev-server`) as a distinct tech from a production `react` build, mirroring `next-dev`/`vue-dev`. `checkReactDevServer.ts` reuses `checkReact.ts`'s existing `@react-refresh`/`injectIntoGlobalHook`/`/@react-refresh` markers for Vite (live-validated), and adds a same-origin same-script-body fetch for webpack's `webpack-dev-server/client`, `webpackHotUpdate`, and `[HMR]` markers (live-validated against a real running `webpack-dev-server` config — both bundler shapes were confirmed live for this issue, unlike `vue-dev`'s untested webpack path). The gate is checked only after `checkReact` matches. `lazyload`'s existing `src/lazyLoad/react/*` pipeline is reused wholesale for `react-dev` once three additive gaps were fixed: `react_followImports.ts`'s import-specifier regexes now match `.jsx`/`.tsx`/`.ts` extensions and an optional trailing query string (Vite dev servers serve component modules under their real source extension, with cache-busting `?v=<hash>` suffixes on vendor deps — neither was previously matched); `outputPath.ts`'s `getSanitizedAssetFilename` now recognizes `.jsx`/`.tsx`/`.ts`, so a genuinely discovered dev-mode source file no longer silently fails to get an output filename; and a new `extractStringKeyedChunkMap` parser in `webpackChunkParsers.ts` handles webpack 5's named-chunk-id chunk-path-builder pattern (`{"name":"hash",...}[chunkId]`), which none of the three existing numeric-id-oriented parsers recognized — this benefits `generic`'s structural discovery too, since the parser is shared. `run`'s tech allow-list and `map`/`analyze`/`detectBundler` remap (`react-dev` → `react`) were extended accordingly. Benchmarked at 100% recovery across both a Vite and a webpack dev-server test app, validated against **two** independent ground-truth sources — `katana` and `jsr-network-bench`'s headless-browser CDP capture (the private-tool-dependent half of the benchmark lives in the private `js-recon-tests` workspace, not this repo). `scripts/dev-server-benchmark.mjs` also gained a `--cache-file`-per-run fix so successive benchmark runs against the same host:port (e.g. piloting two bundlers on the same local port) don't silently serve a prior run's cached responses. (`lazyload`, `run`)
- New `svelte-dev` tech: detects a SvelteKit dev server (always Vite-based — SvelteKit has no webpack dev-server tool-chain) as a distinct tech from a production `svelte` build, mirroring `next-dev`/`vue-dev`/`react-dev`. Unlike Vue/React, SvelteKit's dev server does not inject a `<script src="/@vite/client">` tag into the served HTML at all; `checkSvelteDevServer.ts`'s primary, live-validated marker is instead the `__sveltekit_dev` global assigned inside the app's own inline hydration `<script>` block, with `/@vite/client` (script/link pathname, or an intercepted request URL) as a secondary signal. The gate is checked only after `checkSvelte` matches. A research spike found `lazyload`'s existing `src/lazyLoad/svelte/*` pipeline (plus the shared `react_followImports.ts` ESM-follower it reuses) already recovers most of a dev server's source tree unmodified — the one gap was that SvelteKit's dev-mode per-route node files re-export a route's compiled component directly from its `.svelte` source path (`export { default as component } from "/src/routes/+page.svelte"`), an extension none of the three shared gates recognized: `react_followImports.ts`'s import-specifier regex, `downloadQueue.ts`/`downloadFilesUtil.ts`'s URL-eligibility pre-filter, and `outputPath.ts`'s `getSanitizedAssetFilename` all now recognize `.svelte` (alongside the `.ts` extension already needed for `.ts` route/hook files, which had the same three-gate shape). Once fixed, the crawler follows into the compiled `.svelte` module the same way it already followed `.js`/`.ts` files, recovering the rest of the `.vite/deps`/`@sveltejs/kit` runtime subtree transitively. `run`'s tech allow-list and `map`/`analyze` remap (`svelte-dev` → `svelte`) were extended accordingly. Benchmarked at effectively complete source-file recovery against a live SvelteKit dev server, validated against an independent `katana` crawl. (`lazyload`, `run`)
- New `angular-dev` tech: detects an Angular dev server (`ng serve`) as a distinct tech from a production `angular` build, mirroring `next-dev`/`vue-dev`/`svelte-dev`/`react-dev` (#193). Angular CLI has shipped two structurally different dev-server implementations — the pre-17 `:browser` webpack-dev-server builder, and the Angular 17+ `:application` esbuild builder, which serves dev builds through Vite in middleware mode. `checkAngularDevServer.ts`'s esbuild/Vite markers (`<script type="module" src="/@vite/client">`, `import.meta.hot`, unhashed `main.js`/`polyfills.js` entry filenames) are live-validated against a running `ng serve`; the legacy webpack markers (`webpack-dev-server/client`, a `/sockjs-node/`-pathname URL, unhashed `main.js`/`polyfills.js`/`vendor.js`/`runtime.js` entry filenames) are implemented from spec but not live-validated, since every Angular research app available uses the esbuild builder — same situation `checkVueDevServer.ts`'s webpack markers are in. The gate is checked only after `checkAngularJS` matches. No dedicated crawler was needed: a gap check confirmed `lazyload`'s existing `src/lazyLoad/angular/*` pipeline already recovers Angular's Vite-pre-bundled framework dependency chunks (served from absolute-filesystem-path `/@fs/.../vite/deps/@angular_*.js` routes) via its existing static/dynamic import-following logic, with zero additive fixes. `run`'s tech allow-list and `map`/`analyze` remap (`angular-dev` → `angular`) were extended accordingly. Benchmarked against a live esbuild/Vite-generation dev server via an independent `katana` crawl plus a static import-graph gap check; full Puppeteer-driven E2E validation and the legacy webpack generation are follow-up work (documented in the internal-docs write-up). (`lazyload`, `run`)
...
- `lazyload`/`run` now decode inline (base64 or percent-encoded) `data:` URI sourcemaps found in `//# sourceMappingURL=...` comments — e.g. Vite's `build.sourcemap: 'inline'` or webpack's `devtool: 'inline-source-map'`/`eval-source-map` — instead of treating the reference as a fetchable URL and losing the sourcemap entirely. Handled for every framework the crawler supports: React, Vue, and Svelte (which already scanned for `sourceMappingURL`) gained a `data:` branch; Next.js, Angular, and Nuxt (which had no such scanning before) gained it new, via a shared `discoverSourcemapUrls` helper (Next.js, Angular) or by extending their existing per-file string-analysis pass (Nuxt). Angular and Nuxt also gained the `extractSourceMaps` post-processing step they were missing entirely. New `next_sourcemapUrls` / `angular_sourcemapUrls` methods are visible in `--list-methods` and toggleable via `--include-methods`/`--exclude-methods`. (`lazyload`, `run`)
...
- New Next.js `lazyload` method `next_routerStateForge`: for every route the crawl finds redirecting an unauthenticated request (a session/auth gate), it retries the route as an RSC fetch with a forged `Next-Router-State-Tree` header claiming a known ancestor path segment is already client-rendered, plus the matching `_rsc` cache-busting key (SHA-256 of the four cache-busting inputs, truncated to 96 bits, base64url — not a secret, computable from headers the client itself sends). Some Next.js apps put their only auth check inside a layout component instead of middleware; Next.js skips re-executing that layout's component function when the client's router-state claims it's already mounted, which can return the protected child content — and any JS chunk references inside that Flight payload — for a request with no session at all. A response is only treated as a hit when it's `200`, has an RSC (`text/x-component`) content type, and does not contain a `NEXT_REDIRECT` record. Runs as the crawler's final phase, after brute-force `.map` discovery; opt out per-target via `--exclude-methods next_routerStateForge`. If no full-body ancestor claim succeeds for a candidate (e.g. a tenant-scoped layout the ancestor heuristic doesn't cleanly match), the method also tries marking the leaf segment `metadata-only` instead — this narrower claim can still leak dynamic `generateMetadata()` output even where the full-body claim doesn't apply, and is reported separately from full-body hits. Every forged request now tries the cheap legacy `_rsc` key (32-bit djb2 hash, base36, 5 characters — no SHA-256 needed) before falling back to the strong key; some Next.js builds accept the legacy key as a fallback when the strong-key check fails, so this can skip the crypto step entirely, and which key mode actually worked is reported as `next_routerStateForge_legacyKeyAccepted` in `--research-output` — a signal for which build line is running, since only builds with the legacy-key fallback path accept it at all. Ancestor claims also try common dynamic-route param-name guesses (`id`, `slug`, `org`, `tenant`, `username`) for the last ancestor segment: Next.js's `matchSegment` requires an exact `[paramName, value]` tuple for a dynamic route (e.g. `app/organizations/[org]/layout.js`) — a bare string never matches one — and a black-box crawl can only observe the URL's literal value, not the app's real param name. Chunk-path extraction from a bypassed Flight payload matches the actual wire format (`static/chunks/*.js`, no `_next/` prefix — the client resolves that against its own asset prefix at runtime) rather than assuming one. (`lazyload`)
- `next_routerStateForge`'s dynamic-route param-name guessing is expanded from 5 names (`id`, `slug`, `org`, `tenant`, `username`) to a larger fixed set (`orgId`, `organization`, `tenantId`, `userId`, `user`, `account`, `accountId`, `workspace`, `workspaceId`, `project`, `projectId`, `team`, `teamId`, `handle`, `name`, `key`) and gains a bounded fallback: when every fixed-list guess fails for a candidate ancestor segment, the Flight/RSC response bodies already fetched during those failed attempts are scanned (no extra requests — the bodies are already paid for) for the app's real param name via two anchored patterns — a value-anchored `"key":"value"` match against the ancestor's already-known literal value (Flight serializes plain-object Client Component props this way even though the payload isn't valid JSON/JS), and a dev-mode-only pattern that reads bracketed segment names straight out of embedded webpack module source paths (`.../[paramName]/...`). This fallback only runs after the fixed list fails, and only spends extra requests if harvesting actually finds a name not already tried — new `--rsc-param-bruteforce-limit <n>` (default `20`, `0` disables the fallback) caps how many harvested names get tried per candidate URL. (`lazyload`)
- New Next.js `lazyload` method `next_serverActionIdScan`: after every chunk is downloaded, scans each `.js` file for `createServerReference("<id>", ...)` call sites (matching the same ID-format validation `map`'s AST-based Server Action resolver already uses, `/^[0-9a-f]{40,}$/i`, but on raw chunk text — a lightweight signal available immediately at `lazyload` time, before `map` runs its full resolution) and writes any discovered IDs to `server_action_ids.json` in the target's output directory (`{ actionIdsByFile, allActionIds }`). This is endpoint enumeration only — no action is invoked; a Server Action ID shipped in a public chunk for a page the crawl couldn't reach unauthenticated is itself a disclosure signal (the same shape as the historical CVE-2026-64643 endpoint-ID disclosure). Opt out per-target via `--exclude-methods next_serverActionIdScan`. (`lazyload`)
- New `exploit` subcommand: `js-recon exploit -u <url> --cve <id>` attempts exploitation of known framework CVEs to surface hidden attack surface (e.g. bypassing Next.js middleware auth reveals admin-only JS chunks a normal crawl never sees), rather than just flagging vulnerabilities. `--list-cve` prints every supported CVE grouped by framework and sorted by ID. Covers CVE-2025-29927 and CVE-2026-64645 (Next.js), CVE-2025-30208 and CVE-2026-39363 (Vite), CVE-2026-71320 and CVE-2026-71315/CVE-2026-53721 (Nuxt), CVE-2026-68945 (Angular), and CVE-2025-55182 "React2Shell" (React RSC). Findings write both a rich `exploit.json` (with any newly discovered URLs) and an `EngineOutput`-compatible file consumable by `report`'s new `--exploit-json` flag, which renders exploit findings in the existing HTML/SQLite report alongside `analyze`'s findings. Two of the eight CVEs (`CVE-2026-71320`, `CVE-2025-55182`) are RCE-class — their proof executes a real command server-side via `--exec-cmd` (default `id`); this is intentional, active exploitation, not passive detection, and should only be run against targets you're authorized to test. (`exploit`, `report`)
...
- New `generic` fallback tech in `lazyload` for sites running no supported framework: previously an undetected framework triggered a separate Puppeteer pass just to watch loaded requests and retry framework detection, and if that still found nothing the target was otherwise left with only whatever the browser happened to load. `generic` now recursively crawls the site's own `<a href>`/`<iframe src>` links, plus absolute URLs embedded in any other attribute value such as an `onclick`-driven `window.open(...)` popup (breadth-first, capped by `--max-pages`, downloading JS incrementally per page) rather than stopping at the landing page, since the JS a site's homepage references is rarely all the JS the site has. Unless `-s`/`--strict-scope` is set, the crawl's default scope is now the host reached after following redirects from `-u` (new `--max-redirects` flag, default 20) instead of the unrestricted `*` default used elsewhere. On every page, `generic` extracts `<script src>`/inline scripts/`<link rel="modulepreload">` the same way the framework crawlers do (decoding `data:` URI script sources; skipping `<script type="...">` values that indicate non-JS content, e.g. `application/ld+json`/`speculationrules`), and additionally walks every HTML attribute value, resolving each with the `URL` constructor and — for any URL with a path segment ending in `.js` (catching cachebuster-suffixed paths like `.../beacon.min.js/v124/token` that don't end in `.js` themselves) — confirms it's actually JavaScript via its response `Content-Type` (accepting `text/javascript` per RFC 9239, plus RFC 4329's now-obsoleted `application/javascript`/`application/ecmascript` and other legacy variants) rather than trusting the extension alone. `generic` is also now seeded with every request tech detection's own Puppeteer session intercepted while live-rendering the page — this catches JS requested only because a runtime-injected script asked for it (e.g. a bot-mitigation script self-injecting its own next stage via `element.innerHTML`), which no static-fetch crawl could ever see. A new `--strings` flag chains the `strings` module into the crawl to catch JS referenced only as a string literal inside an already-downloaded file (e.g. a plugin config's `"pdfWorker": "https://.../pdf.worker.js"`), resolving each extracted string against the URL the file it was found in was itself downloaded from, and recursing (`--strings-max-iterations`, default 5) until a pass finds nothing new. `run` continues to only download JS for this tech, same as any other tech outside its supported list. (`lazyload`)
- `generic` tech now detects content stagnation on effectively unbounded sites (blogs, news feeds) that keep serving byte-identical JS under a different, cache-busted URL on every page — a pattern the existing URL-level dedup doesn't catch, which previously burned the full `--max-pages`/`--lazyload-timeout` budget on a site whose real JS surface was already fully mapped. New flags `--stagnation-timein <mins>` (default 30, `0` disables the feature), `--stagnation-percentage <percent>` (default 80), and `--stagnation-monitor <mins>` (default 1) control a debounced check: once one content hash accounts for `--stagnation-percentage`% of everything discovered so far, the crawl arms a pending state rather than stopping immediately — if a genuinely new content hash appears within the next `--stagnation-monitor` interval, monitoring resets; only if the same dominant hash persists with no new content does the crawl stop early. New exit code 30 if `--stagnation-timein` exceeds `--lazyload-timeout`, since stagnation monitoring could then never trigger before the whole crawl times out. (`lazyload`)
- New `--verbose` flag (`lazyload`, `run`) gates the file-write error message previously always printed when a downloaded JS chunk fails to write to disk (`[!] Failed to write file: <path>`). It is now silent by default and only shown with `--verbose`; no other error messages are affected. (`lazyload`)
- New `--determine-compatible-version`/`--apply-compatible-versions` flags (`analyze`) report or auto-correct the `js_recon_version`/`js_recon_max_version` a rule file actually requires, computed from a hand-maintained map (`src/utility/ruleVersionMap.ts`) of which tool version introduced each schema/engine feature (tech values, rule types, step sub-fields). Previously rule authors had to guess the compatibility string by hand, which could under- or over-restrict a rule relative to what it actually uses. Rules also now support an optional `js_recon_max_version` field for features that have since been retired; version-compatibility mismatches reported by `--determine-compatible-version` reuse exit code 20 ("invalid rules found"), same as schema errors. (`analyze`)
...
- `downloadFilesUtil.ts`/`DownloadQueue` (used by every framework crawler except `generic`) determined a downloaded file's on-disk name by checking only the URL's final `/`-separated path segment for a recognized extension, with a fallback loop that scanned every segment but was unreachable in practice — it lived in a `catch` block after a regex `.match()` call that returns `undefined` on no match rather than throwing. URLs where a token or cachebuster segment follows the actual filename (e.g. `.../beacon.min.js/v124/token`) were logged as `Could not determine filename for URL: ... Skipping.` and never downloaded, even though `generic`'s own downloader already handled this exact shape. Filename resolution is now handled by dev's `getSanitizedAssetFilename`/`resolveAssetOutputDirectory` helpers (`src/utility/outputPath.ts`), which already scan every path segment for the first one ending in a recognized extension. (`lazyload`)
- `run`'s Ctrl-C interrupt menu could show a stray leftover message from the step that was in flight when the interrupt fired: `Promise.race` doesn't cancel the loser, so the interrupted step's own promise kept running in the background and could still call `console.log`/`console.error` while the menu/prompt was on screen, interleaving its output with the prompt. All ad-hoc `console.log`/`console.error` calls across the CLI have been replaced with a new `printMsg(MSG.Level, message)` helper (`src/utility/printMsg.ts`) that funnels every message through one call site; `run/interruptHandler.ts` now calls `lockPrintMsg()` before showing the menu and `unlockPrintMsg()` after the choice is read, so any message from an orphaned in-flight step is silently discarded instead of printing over the prompt. (`run`)
- `checkAssignmentExist`/`findMemberExpressionAssignment` (used by rules like `detect_postMessage_innerHtml_sink` and `detect_postMessage_function_href`) crashed `analyze`/`run` entirely with `SES_UNHANDLED_REJECTION: Couldn't find a Program` when the resolved `postMessage` handler's sensitive assignment was nested inside an `if` block. The helper called `@babel/traverse`'s scope-aware form with a raw `File`/`Program` AST node passed in place of a real `Scope` instance, which broke Babel's internal scope-chain construction for some node shapes. Fixed by traversing with `noScope: true` instead, since the helper never reads `path.scope`. (`analyze`)
- The response cache (`makeReq.ts`) stored a live `Headers` object directly into data later passed to `JSON.stringify()`. `Headers` has no own enumerable properties, so this silently serialized to `{}` on every cache write, discarding all response headers (not just `Content-Type`) for any URL cached earlier in a run. Any header-dependent check that hit a cached URL a second time (e.g. `generic`'s Content-Type-based JS confirmation) could silently fail depending on whether something upstream had already cached that URL. Fixed by converting to a plain object (`Object.fromEntries(headers.entries())`) before serialization. (`utility`)
- `checkVueJS.ts`'s last-resort tech-detection fallback scanned every externally-referenced `<script src>`, including cross-origin third-party scripts, for the substring `__vue`. A third-party widget that internally bundles Vue for its own UI could cause a site with no Vue anywhere in its own code to be misclassified as Vue.js. The fallback now only scans same-origin scripts. (`lazyload`)
- `strings`: a single file that fails to parse as JS (e.g. non-JS content saved with a `.js` extension) no longer aborts extraction for every other file in the scan — the parse error is now caught, logged, and skipped per-file. (`strings`)
- `lazyload --strings` (generic tech): each recursive strings-discovery pass rescans the same (ever-growing) output directory, since the `strings` module has no incremental-scan mode. Every already-confirmed URL from a prior pass was being re-confirmed via a fresh live `GET` request on every subsequent pass, compounding badly on large multi-hundred-page crawls. Already-known URLs are now filtered out before the Content-Type confirmation step, not after. (`lazyload`)
- `DownloadQueue`'s write-error log interpolated the raw error object (`${writeErr}`). For a `prettier.format()` parse failure the thrown `SyntaxError`'s message has Babel's codeframe appended after the first line, and on minified JS (a single giant line) that codeframe is the entire file — dumping the whole source to the terminal instead of just the error. The logged message is now truncated to the first line (`String(writeErr).split("\n")[0]`), e.g. `SyntaxError: Missing semicolon. (2:4)`. (`lazyload`)
- `map`'s per-file size guard (`MAX_MAP_FILE_SIZE_BYTES`, present identically across `getReactConnections.ts`/`getViteConnections.ts`/`getAngularConnections.ts` and the shared fetch/XHR/HTTP-client resolvers) skipped any file over 1.5 MB with no fallback chunk, so a single large un-split app bundle vanished from `mapped.json` entirely — `refactor` then ran against whatever small chunks were left instead of the app, reporting near-zero recovery. Raised the cap to 8 MB; the guard's purpose is capping the many-large-files case (ad-heavy sites), not a single reasonably-large main bundle. Also hardens `run`'s CS-MAST-S bundler detection (react-webpack vs react-vite): adds a cheap literal bundler-runtime-marker pre-check that short-circuits the structural comparison when unambiguous, and raises the random sample size to cover the current signature bucket, since undersampling could previously flip which tech "won" between identical runs against the same bundle. (`map`, `run`)
- The startup banner logo rendered visibly distorted (stretched, overlapping the following text) in Kitty-protocol terminals (Kitty, Ghostty). Root cause was in `@shriyanss/cli-print-img`'s Kitty renderer, which specified both `c` (columns) and `r` (rows) in the same escape sequence — per the Kitty Graphics Protocol, giving both stretches the image to fill that exact cell box instead of preserving its aspect ratio, and it also suppressed cursor movement without advancing by the actual rendered height. Bumped `@shriyanss/cli-print-img` to `1.1.1`, which fixes both issues upstream. (`cli`, `banner`)
- `map`'s Next.js webpack chunk-code extraction (`getWebpackConnections.ts`) stripped a chunk's numeric-ID object-literal prefix (e.g. `0:function(e,t,n){...}` → `function webpack_0(e,t,n){...}`) via two regexes that both assumed the minifier inserts whitespace between the colon/`function` keyword and the following token. Real-world minified output commonly omits that whitespace, so the prefix was left in place — invalid syntax that failed to parse downstream in `refactor -t next-webpack`, silently dropping the module instead of processing it. Also affected `refactor`'s CS-MAST library classification, since a chunk that never parses can never be checked against the signature set either. Fixed by relaxing both regexes' whitespace assumption (`\s+` → `\s*`, plus a missing `\s` escape in the arrow-form regex). Verified against a real-world target: eliminated all parse failures on a previously 96%-failing chunk set and raised its library-classification rate from ~3% to 94%. Follow-up: the arrow-form regex still required a literal `(` immediately after the numeric-ID colon, missing JS's single-parameter arrow shorthand with no parens (`3562:e=>{...}`, common in lodash-style utility functions). Fixed by matching either `(` or a bare identifier followed by `=>` via lookahead, stripping only the `id:` prefix and leaving the arrow's own param syntax untouched. (`map`)

**Note:** `post-1.4.1`'s independent proxy rework (renaming `api-gateway` to `proxy`, subcommand-per-provider design) was superseded during the `post-1.4.1` → `dev` promotion merge by `dev`'s own concurrent proxy rework, which reached the same rename via a single `proxy` command with `--proxy-method <aws|socks|http|oxylabs>` plus CloudFront/Cloudflare firewall detection, crash-safe AWS Gateway cleanup, and full test coverage — see the `1.4.1-beta.2` entry below for the proxy rename actually shipped. (`proxy`)

## 1.4.4 - 2026-08-03

### Added

- `run`, `lazyload`, `fingerprint`: `-u/--url` now accepts one URL, the flag repeated multiple times, a comma-separated list of URLs, or a target file (previously a single URL or a single target file). Precedence across environment variable, CLI flag, and YAML config is preserved for all forms. (`run`, `lazyload`, `fingerprint`)
- `run`: output directory handling is now non-destructive by default. An existing **empty** output directory is reused; an existing **non-empty** one is left untouched and js-recon allocates the next available sibling (`output-2`, `output-3`, ...) instead. `--output-overwrite` is now scoped to canonical, js-recon-owned directories — it rejects symlinked or otherwise protected paths rather than overwriting them. Concurrent runs targeting the same output coordinate via PID/host-aware atomic locks with stale-owner recovery and signal cleanup, so simultaneous invocations no longer race on the same directory. (`run`)
- Added application-level YAML configuration. On first use, js-recon creates `~/.config/js-recon/config.yaml` (restrictive permissions) with a key for every command option; an alternate file can be selected with the new top-level `--config <file>` flag (before the subcommand) or `JS_RECON_CONFIG`. Values resolve as environment variable → CLI flag → YAML config → built-in default, using `JS_RECON_<COMMAND>_<OPTION>` for any command option (e.g. `JS_RECON_RUN_THREADS=8`). Targets can also be configured as a YAML list under `commands.<command>.url`; an env value or CLI `-u` replaces the whole list per the usual precedence. See `config.dist.yaml` for the full schema. The existing command-specific `proxy --config` / `mcp --config` flags are unaffected — place the application-level `--config` before the subcommand to avoid ambiguity. (`config`, `run`, `lazyload`, `fingerprint`)
- `run`/`lazyload`: added opt-in Oxylabs recovery for strongly corroborated CDN/WAF blocks, enabled via the new `--oxylabs-waf-fallback` flag once `oxylabs.username`/`oxylabs.password` are set in an explicitly selected YAML config (`JS_RECON_OXYLABS_USERNAME`/`_PASSWORD`/`_COUNTRY` override the YAML values unless `--ignore-proxy-env` is set). Direct requests remain the default path; Oxylabs is only used after direct header-based strategies are exhausted, bounded by new `--oxylabs-fallback-max-requests` (default `10`, per origin), `--oxylabs-fallback-max-total` (default `100`, per run), and `--oxylabs-fallback-max-origins` (default `25`, per run) flags. (`run`, `lazyload`, `proxy`)
- React/webpack chunk downloads now show a responsive, TTY-aware aggregate progress bar; piped/CI output stays compact with no per-file error spam. Download/response/formatting/write failures are tracked independently and cooperative cancellation is honored throughout discovery, requests, browser fallback, cache access, and post-processing. URL host/path/filename sanitization is now centralized to prevent malformed JS-expression paths, queries, ports, IPv6 hosts, traversal attempts, or hostile source-map entries from escaping or overwriting output boundaries. (`lazyload`, `run`)
...
- `strings`: the main per-file parse loop had no `try/catch`, unlike the `___subsequent_requests` branch a few lines above (and unlike `sourcemaps`/`cs-mast`). A single malformed or truncated `.js` file among otherwise-valid files threw an uncaught `SyntaxError` and aborted the entire strings extraction with no output at all — including for the `run` pipeline's steps 2/4/4.6. Unparseable files are now skipped with a warning instead of crashing the run. (`strings`, `run`)
- `run`: requesting `-o <path>` where `<path>` already exists as a regular file (not a directory) silently fell through to the output-reservation retry loop and created `<path>-2` instead of reporting the problem, unless `--output-overwrite` was passed — with `--output-overwrite` it already errored correctly, but without it (the default) there was no indication the requested path wasn't a directory at all. A file at the output path is now rejected outright regardless of `--output-overwrite`. (`run`)
- `lazyload`/`run`: the output-directory host-nesting heuristic (used to avoid double-nesting a batch target's already-host-owned per-target directory) was applied unconditionally at every call site instead of only the batch per-target root it was written for. A single-target `-o <dir>` run whose directory basename happened to match the sanitized target host silently collapsed the output layout instead of nesting under `<dir>/<host>/...`. The heuristic is now gated behind an explicit batch-only flag threaded through lazyload/crawler/download-queue internals, so single-target runs always nest correctly. (`lazyload`, `run`)
- `proxy`: the AWS API Gateway resource path (`resourcePathPart`) mixed a per-call random UUID into its hash, so it was never the same twice for the same URL — the existing-resource reuse lookup could never match a previously-provisioned resource, and every proxied request via the `aws` proxy method created a brand-new API Gateway resource instead of reusing one. It's now derived from the URL alone (still SHA-256) so resource reuse works across repeat requests to the same URL. (`proxy`)
...
- `docker`: the official images now start as root and drop to the unprivileged `pptruser` at runtime via a new `docker-entrypoint.sh`, after `chown`-ing the bind-mounted output directory. Previously, a bind-mounted `output/` directory created by Docker (or Kubernetes) with host/root ownership was unwritable by the container's non-root user, causing the pipeline to fail before processing any target. This makes `docker run -v $(pwd)/output:/home/pptruser/output ...` work out of the box regardless of who owns the directory on the host, with no manual `chown` step required. `HOME` is now pinned to `/home/pptruser` so path resolution (Puppeteer's Chrome cache, the rules cache, etc.) stays consistent whether a process runs as root or as `pptruser` — needed for invocations that bypass the entrypoint via `--entrypoint`. (`docker`)
...
- The `rimraf` override above broke `puppeteer-extra-plugin-user-data-dir` (a transitive dependency of `puppeteer-extra-plugin-stealth`), which still calls the old `rimraf(path, opts, callback)` API; `rimraf@6` no longer exports a default callable function, causing `TypeError: rimraf is not a function` whenever a Puppeteer session disconnected during `run`. Patched via `patch-package` to use `fs.rm` instead of `rimraf` in that plugin. (`run`, `deps`)
- `run` (batch mode, `-u <file>`): the per-URL loop had no `catch` around `processUrl()`, so an unhandled exception for one target propagated into the outer `finally`'s unconditional `process.exit(process.exitCode ?? 0)` — terminating the process with a false "success" exit code before Node's default uncaught-exception reporting could print anything, and silently abandoning every subsequent target in the batch. Each target's `processUrl()` call is now wrapped in its own `try/catch`: a failure is logged, `process.exitCode` is set to `1`, and the loop continues to the next URL. (`run`)
